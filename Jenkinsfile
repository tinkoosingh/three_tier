pipeline {
    ...
    stage('Deploy'){
        steps{
            ...
            sh 'kubectl apply -f mysql_dep --token ${KUBE_TOKEN} --server https://localhost:6443'
            sh 'kubectl apply -f configmap.yml --token ${KUBE_TOKEN} --server https://localhost:6443'
            sh 'kubectl apply -f app_deployment.yml --token ${KUBE_TOKEN} --server https://localhost:6443'
            sh 'kubectl get all --token ${KUBE_TOKEN} --server https://localhost:6443'
        }
    }
    }
}
